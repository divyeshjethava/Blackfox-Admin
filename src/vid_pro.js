...
speechSynthesis.speak(utterance);
 ...
 const pro1 = ref(db,'Products/' +pid.value+randomNum);
 const cat1 = ref(db,ctg+'/'+stg+'/'+ pid.value+randomNum);
 const videoRef = ref(db,'video/'+ vid)
 const all = ref(db,ctg+'/'+'All'+'/'+ pid.value+randomNum);
...
sPcial_btn_pro.addEventListener('click',()=>{
  ...
  speechSynthesis.speak(utterance);
   ...
   const pro1 = ref(db,'B&W Collection/' +pid.value+randomNum);
